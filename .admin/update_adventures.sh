...
# Split the community adventures by sections
csplit ../wiki.wiki/Community-Adventures.md '/^# /' '{*}' --prefix csplit-sections- > /dev/null
	...
	if [[ "$section_name" != "" && "$section_name" != "Appendix" ]]; then
		...
		for module in csplit-modules-* ; do
			...
			if [[ "$name" == "$section_name" ]] ; then
				cat $module >> adventures.md
				continue
			fi
			if [[ "$name" == "(Adventure Name)" ]] ; then continue; fi
			...
			if [[ "$description" != "" || "$url" != "" ]] ; then
				...
				echo "## [$name]($link)" >> adventure.md
				echo $description >> adventure.md
				echo "" >> adventure.md
			else
...
cat header.md adventure.md > README.md
rm adventure.md
